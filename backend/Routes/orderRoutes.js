  ...
  try {
    ...
    for (const orderedItem of items) {
      ...
      for (const recipeIngredient of recipe.ingredients) {
        ...
        const currentIngredient = await Ingredient.findById(recipeIngredient.ingredient._id);
        const newStock = currentIngredient.currentStock - deductAmount;

        await Ingredient.findByIdAndUpdate(
          recipeIngredient.ingredient._id,
          { $set: { currentStock: Math.max(0, newStock) } } // ← never go below 0
        );
